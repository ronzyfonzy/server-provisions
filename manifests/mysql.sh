...
function _install {
	...
	installApt mysql-server-5.6
	installApt mysql-server-5.6 mysql-client-5.6

	printf "Importing timezones\n"

	mysql_tzinfo_to_sql /usr/share/zoneinfo | mysql -u ${DB_USER} -p${DB_PASSWORD} mysql
}
...
function _uninstall {
	printf "uninstall\n"
	uninstallApt mysql-server-5.6 mysql-client-5.6
}
